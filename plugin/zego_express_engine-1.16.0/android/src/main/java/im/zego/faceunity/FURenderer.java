...
import im.zego.faceunity.entity.Effect;
import im.zego.faceunity.entity.FaceMakeup;
import im.zego.faceunity.entity.Filter;
import im.zego.faceunity.entity.MakeupItem;
import im.zego.faceunity.utils.Constant;
...
public class FURenderer implements OnFUControlListener {
    ...
    private volatile static float mBlurLevel = 0.0f;//磨皮程度
    private volatile static float mColorLevel = 0.0f;//美白
    ...
    private volatile static float mCheekV = 0.0f;//V脸
    private volatile static float mCheekNarrow = 0f;//窄脸
    private volatile static float mCheekSmall = 0f;//小脸
    private volatile static float mEyeEnlarging = 0f;//大眼
    private volatile static float mIntensityChin = 0.0f;//下巴
    private volatile static float mIntensityForehead = 0.0f;//额头
    private volatile static float mIntensityMouth = 0.0f;//嘴形
    private volatile static float mIntensityNose = 0.0f;//瘦鼻
...
//        Log.i("faceUtil","fuIsTracking->" + faceunity.fuIsTracking());
        ...
        if (isNeedUpdateFaceBeauty && mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX] > 0) {
            ...
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.BLUR_LEVEL, 6 * ModelParams.sBlurLevel);
            //color_level 美白 范围0~1 SDK默认为 0.2
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.COLOR_LEVEL, ModelParams.sColorLevel);
            //red_level 红润 范围0~1 SDK默认为 0.5
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.RED_LEVEL, ModelParams.sRedLevel);
            //eye_bright 亮眼 范围0~1 SDK默认为 0
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.EYE_BRIGHT, ModelParams.sEyeBright);
            //tooth_whiten 美牙 范围0~1 SDK默认为 0
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], BeautificationParams.TOOTH_WHITEN, ModelParams.sToothWhiten);

            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "remove_pouch_strength", ModelParams.sMicroPouch);
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "remove_nasolabial_folds_strength", ModelParams.sMicroNasolabialFolds);
