...
@property (nonatomic, strong) ZGCaptureDeviceCamera *deviceCamera;
...
- (void)enableCamera:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (nil != _deviceCamera) {
        if (enable) {
            [[ZegoCustomVideoCaptureManager sharedInstance] setCustomVideoCaptureDelegate:self channel:channel];
        } else {
            [[ZegoCustomVideoCaptureManager sharedInstance] setCustomVideoCaptureDelegate:nil channel:channel];
        }
        
        [_deviceCamera enableCamera:enable];
    } 
...
- (void)useFrontCamera:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (_deviceCamera) {
        [_deviceCamera switchCamera:enable];
    } else {
...
- (void)enableCustomVideoCapture:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if (enable) {
        _deviceCamera = [[ZGCaptureDeviceCamera alloc] initWithPixelFormatType:kCVPixelFormatType_32BGRA];
        _deviceCamera.delegate = [ZegoCustomVideoCaptureManager sharedInstance];
...
- (void)onStart:(ZegoCustomVideoCaptureClient*)client {
    [_deviceCamera startCapture];
}

- (void)onStop {
    [_deviceCamera stopCapture];
}


#pragma mark - beautiful 
- (void)updateBeautiful:(FlutterMethodCall *)call result:(FlutterResult)result {
    result(nil);
}
